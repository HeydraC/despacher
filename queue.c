...
void push(queue *q, struct process p) {
    ...
    if (q->first == NULL) {
        q->first = new_node;
        q->last = new_node;
    } else { // Si la cola no está vacía, añadir al principio (FCFS de los que ya están)
...
void SuperColaPush(SuperCola *sc, struct process process_to_add) {
    ...
    if (process_to_add.priority == 0) { // Procesos de Tiempo Real 
        process_to_add.quantum = process_to_add.procTime;
    } else { // Procesos de Usuario (P1, P2, P3)
